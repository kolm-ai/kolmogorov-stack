...
test('1b. backend spec records MCP closures and scopes remaining interop work', () => {
  ...
  assert.match(spec, /CLOSED W982: Bind caller, agent, MCP session, and upstream response provenance into signed MCP receipts/i);
  assert.match(spec, /CLOSED W983: Chain the full MCP session transcript into signed v3 receipts/i);
  assert.match(spec, /external verifier ecosystem interop and public test-vector adoption/i);
  assert.doesNotMatch(spec, /\[major\] No production upstream MCP client registry\/executor/);
  assert.doesNotMatch(spec, /Full MCP session transcript continuity is still not first-class/i);
