...
test('W684 drift-alert source pins bounded sketch and envelope controls', () => {
  ...
  assert.match(pkg.scripts['verify:depth'], /verify:finetune-frameworks && npm run verify:drift-alert && npm run verify:drift-alert-w813 && npm run verify:failure-modes-w745 && npm run verify:openai-finetune-importer && node --test/);
});
