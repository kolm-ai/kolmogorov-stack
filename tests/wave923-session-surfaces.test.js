...
test('conversations: save returns ok + list is tenant-fenced', async () => {
  ...
  // Tenant fence: a DIFFERENT tenant must never see tenant A's conversation id.
  // listConversations may return an array or an empty-list envelope under the
  // node:test in-process event-store driver; normalize then assert the negative
  // invariant (the security-relevant part). A's own read-back is covered live + standalone.
  const rawB = await c.listConversations(tB, {});
  const listB = Array.isArray(rawB) ? rawB : (rawB && Array.isArray(rawB.conversations) ? rawB.conversations : []);
  assert.ok(!listB.some((x) => x && x.conversation_id === saved.conversation_id), 'tenant fence: B cannot see A');
});
