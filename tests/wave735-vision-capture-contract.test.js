...
test('W735 vision capture is wired into direct depth verification', () => {
  ...
  assert.match(
    ...
    /verify:video-capture && npm run verify:vision-capture && npm run verify:vlm-bakeoff && npm run verify:website-status && npm run verify:zip-large && node scripts\/audit-sota-readiness\.cjs/,
  );
