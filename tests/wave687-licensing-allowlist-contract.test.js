...
test('W687 licensing allowlist pins bounded evidence controls and depth wiring', () => {
  ...
  assert.match(pkg.scripts['verify:depth'], /verify:package-release && npm run verify:licensing-allowlist && npm run verify:savings-routes && npm run verify:scim-provisioning && npm run verify:trend-extract && npm run verify:verticals && npm run verify:video-bakeoff && npm run verify:video-capture && npm run verify:vision-capture && npm run verify:vlm-bakeoff && npm run verify:website-status && npm run verify:zip-large && npm run verify:python-onnx-text && npm run verify:fal-backend && node scripts\/audit-sota-readiness\.cjs/);
});
