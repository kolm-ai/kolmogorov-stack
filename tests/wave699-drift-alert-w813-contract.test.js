...
test('W699 source pins W813 alert wrapper controls and package wiring', () => {
  ...
  assert.match(pkg.scripts['verify:depth'], /npm run verify:drift-alert && npm run verify:drift-alert-w813 && npm run verify:failure-modes-w745 && npm run verify:openai-finetune-importer && npm run verify:poisoning-orchestrator && node --test/);
});
