...
const CANONICAL = 'npm i -g github:sneaky-hippo/kolmogorov-stack';
...
test('W494 #2 - first-party install docs use the GitHub npm source path', () => {
  for (const relPath of [
    ...
    assert.ok(html.includes('github:sneaky-hippo/kolmogorov-stack'), `${relPath} must show source install`);
  }
...
test('W494 #4 - marketing copy and CLI help name the honest source install', () => {
  ...
  assert.ok(why.includes('npm install -g github:sneaky-hippo/kolmogorov-stack'));
  assert.equal(why.includes('Homebrew, winget, apt'), false);

  const cli = read('cli/kolm.js');
  assert.ok(cli.includes('npm i -g github:sneaky-hippo/kolmogorov-stack'));
