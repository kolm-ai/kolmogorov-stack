...
test('W940 package wiring makes the daemon connector matrix a depth and control-file gate', () => {
  ...
  assert.match(pkg.scripts['build:control-files'], /build:cli-command-matrix && npm run build:daemon-connector-matrix && npm run build:file-ledger/);
  assert.match(pkg.scripts['verify:control-files'], /verify:cli-command-matrix && npm run verify:daemon-connector-matrix && npm run verify:file-ledger/);
