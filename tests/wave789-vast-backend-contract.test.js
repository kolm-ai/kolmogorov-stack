...
test('W789 vast backend is wired into direct depth verification', () => {
  ...
  assert.match(
    ...
    /verify:fal-backend && npm run verify:vast-backend && npm run verify:browser-extension-popup && npm run verify:langchain-package-manifest && node scripts\/audit-sota-readiness\.cjs/,
  );
