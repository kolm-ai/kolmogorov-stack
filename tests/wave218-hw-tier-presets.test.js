...
test('W218 #5 - cmdCompile parses --tier and sets KOLM_HW_TIER + base-model', () => {
  // The injection point is the --tier preset block inside cmdCompile, after the
  // --gate / --target-profile / --group / --cloud handling. cmdCompile has grown
  // since W218 (more pre-flight flags inserted ahead of --tier), so the slice
  // window is sized to reach the --tier block wherever it lands in the function.
  const cmpIdx = CLI_SRC.indexOf('async function cmdCompile(');
  const body = CLI_SRC.slice(cmpIdx, cmpIdx + 20000);
