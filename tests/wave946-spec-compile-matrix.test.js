...
test('W946 package wiring makes the spec compile matrix a depth and control-file gate', () => {
  ...
  assert.match(pkg.scripts['build:control-files'], /build:distill-pipeline-matrix && npm run build:spec-compile-matrix && npm run build:file-ledger/);
  assert.match(pkg.scripts['verify:control-files'], /verify:distill-pipeline-matrix && npm run verify:spec-compile-matrix && npm run verify:file-ledger/);
