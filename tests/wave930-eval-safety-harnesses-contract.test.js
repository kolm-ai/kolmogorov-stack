...
test('W930 #8 - depth verification runs eval safety harnesses before SOTA audit', () => {
  ...
  assert.match(
    ...
    /verify:distribution-manifests && npm run verify:eval-safety-harnesses && npm run verify:worker-safety-contracts && npm run verify:compute-backends && node scripts\/audit-sota-readiness\.cjs/
  );
});
