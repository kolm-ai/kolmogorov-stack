...
test('W938 package wiring makes the auth boundary matrix a depth and control-file gate', () => {
  ...
  assert.match(pkg.scripts['build:control-files'], /build:api-contract-matrix && npm run build:auth-boundary-matrix && npm run build:cli-command-matrix && npm run build:daemon-connector-matrix && npm run build:quantize-worker-matrix && npm run build:file-ledger/);
  assert.match(pkg.scripts['verify:control-files'], /verify:api-contract-matrix && npm run verify:auth-boundary-matrix && npm run verify:cli-command-matrix && npm run verify:daemon-connector-matrix && npm run verify:quantize-worker-matrix && npm run verify:file-ledger/);
  assert.match(pkg.scripts['verify:depth'], /verify:router-contract && npm run verify:api-contract-matrix && npm run verify:auth-boundary-matrix && npm run verify:cli-command-matrix && npm run verify:daemon-connector-matrix && npm run verify:meta-routes/);
