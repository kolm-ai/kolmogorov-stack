...
test('W948 package wiring makes the artifact matrix a depth and control-file gate', () => {
  ...
  assert.match(pkg.scripts['build:control-files'], /build:data-curate-matrix && npm run build:artifact-matrix && npm run build:file-ledger/);
  assert.match(pkg.scripts['verify:control-files'], /verify:data-curate-matrix && npm run verify:artifact-matrix && npm run verify:file-ledger/);
  assert.match(pkg.scripts['verify:depth'], /verify:spec-compile-matrix && npm run verify:data-curate-matrix && npm run verify:artifact-matrix && npm run verify:meta-routes/);
