...
test('W710 meta routes expose a bounded, versioned contract and depth verifier', () => {
  ...
  assert.match(pkg.scripts['verify:depth'], /verify:router-contract && npm run verify:api-contract-matrix && npm run verify:auth-boundary-matrix && npm run verify:meta-routes && npm run verify:governance-packets/);
