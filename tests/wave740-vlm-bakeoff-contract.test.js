...
test('W740 VLM bakeoff is wired into direct depth verification', () => {
  ...
  assert.match(
    ...
    /verify:vision-capture && npm run verify:vlm-bakeoff && npm run verify:website-status && npm run verify:zip-large && npm run verify:python-onnx-text && node scripts\/audit-sota-readiness\.cjs/,
  );
