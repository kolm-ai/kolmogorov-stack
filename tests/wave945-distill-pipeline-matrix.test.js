...
test('W945 package wiring makes the distill pipeline matrix a depth and control-file gate', () => {
  ...
  assert.match(pkg.scripts['build:control-files'], /build:wrapper-cli-matrix && npm run build:distill-pipeline-matrix && npm run build:file-ledger/);
  assert.match(pkg.scripts['verify:control-files'], /verify:wrapper-cli-matrix && npm run verify:distill-pipeline-matrix && npm run verify:file-ledger/);
  assert.match(pkg.scripts['verify:depth'], /verify:wrapper-cli-matrix && npm run verify:distill-pipeline-matrix && npm run verify:meta-routes/);
