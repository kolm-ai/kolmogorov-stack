...
test('W700 source pins W745 bounds, digest contract, and package wiring', () => {
  ...
  assert.match(pkg.scripts['verify:depth'], /verify:drift-alert-w813 && npm run verify:failure-modes-w745 && npm run verify:openai-finetune-importer && npm run verify:poisoning-orchestrator && node --test/);
});
