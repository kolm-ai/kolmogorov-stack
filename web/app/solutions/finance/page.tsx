import type { Metadata } from "next";
import Link from "next/link";

import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import {
  Card,
  CardKicker,
  CardTitle,
  CardDescription,
} from "@/components/ui/card";
import { CheckIcon, ShieldIcon, LogIcon, KeyIcon } from "@/components/icons";

export const metadata: Metadata = {
  title: "For finance and fintech AI vendors",
  description:
    "Your agent cleared the demo, then stalled in a financial institution's security review. Hand the vendor-security, model-risk and third-party-risk teams a signed evidence report they verify offline against your own key - mapped to SOC 2, the NIST AI RMF and the model-risk language of SR 11-7. The four-to-eight-week review compresses back to days.",
};

const FRAMEWORKS = [
  "SOC 2 TSC",
  "NIST AI RMF",
  "SR 11-7 model risk",
  "EU AI Act",
  "OWASP LLM / Agentic Top 10",
  "MITRE ATLAS",
];

const TRUST = [
  "Ed25519-signed",
  "Verified offline against your key",
  "Mapped to SOC 2, NIST AI RMF and SR 11-7 model-risk language",
];

const METRICS = [
  {
    n: "4 to 8 wks",
    l: "how long an agent review runs once vendor security, model risk and third-party risk are all in the loop",
  },
  {
    n: "3 desks",
    l: "the security reviewer, the model-risk function and the third-party-risk team, each asking for evidence they can check",
  },
  {
    n: "Days",
    l: "the same review when you walk in with a signed report they verify themselves",
  },
];

const ASKS = [
  {
    kicker: "Over-permissioned",
    title: "Least privilege over money and account data",
    body: "An agent that can move funds or read account data is judged by everything its credentials can reach. Reviewers flag every scope the agent holds but never uses, and any key shared across customer boundaries.",
    map: "ASR-1 · least privilege",
    icon: ShieldIcon,
  },
  {
    kicker: "Tamper-evidence",
    title: "An audit trail an examiner can trust",
    body: "When an incident or an examiner reconstructs what your agent did, the record has to be append-only and hash-chained. A log that can be edited after the fact is not evidence, and SR 11-7 expects the model's activity to be documented.",
    map: "ASR-2 · audit trail",
    icon: LogIcon,
  },
  {
    kicker: "Data egress",
    title: "Account data never leaves without redaction",
    body: "Every external destination is enumerated, every sub-processor accounted for, and sensitive fields are redacted before they cross the boundary. The reviewer reads exactly where customer data can go.",
    map: "ASR-3 · data egress",
    icon: KeyIcon,
  },
  {
    kicker: "Model provenance",
    title: "Provenance for the model-risk file",
    body: "The model-risk function has to place your agent in its inventory. The report enumerates model and dependency provenance, so they can validate and document it the way SR 11-7 asks, without guessing.",
    map: "ASR-5 · provenance",
    icon: CheckIcon,
  },
];

const HANDOFF = [
  {
    title: "One canonical, signed object",
    body: "Key-sorted, whitespace-free JSON, signed with Ed25519. The signature covers these exact bytes, so a downgraded finding or an inflated score is self-evident the instant the reviewer checks it.",
  },
  {
    title: "Your key travels in the report",
    body: "The signature and the public key it was made with ship together. The bank's team needs nothing from us: the verifier runs offline against the key inside the report, no account, no upload.",
  },
  {
    title: "Your questionnaire, answered with evidence",
    body: "Every line of the vendor security questionnaire is backed by a signed finding mapped to a control the reviewer already enforces, instead of a self-attested checkbox they have to take on faith.",
  },
];

const CROSSWALK = [
  {
    id: "ASR-1",
    name: "Least privilege",
    checks: "Scopes the agent holds versus the scopes it uses",
    maps: "SOC 2 CC6 · NIST MANAGE-1 · OWASP ASI",
  },
  {
    id: "ASR-2",
    name: "Audit trail",
    checks: "Append-only, hash-chained, retained activity log",
    maps: "SOC 2 CC7 · SR 11-7 documentation · EU AI Act Art.12",
  },
  {
    id: "ASR-3",
    name: "Data egress",
    checks: "Destinations, approved sub-processors, redaction",
    maps: "OWASP LLM02 · EU AI Act Art.10",
  },
  {
    id: "ASR-4",
    name: "Injection",
    checks: "Instruction hijack, indirect injection, guardrail bypass",
    maps: "OWASP LLM01 · MITRE ATLAS",
  },
  {
    id: "ASR-5",
    name: "Provenance",
    checks: "Model and dependency provenance",
    maps: "SR 11-7 model inventory · ISO 42001 · NIST MAP-1",
  },
  {
    id: "ASR-6",
    name: "Evidence",
    checks: "Signed, logged, offline-verifiable report",
    maps: "SOC 2 CC7 · ISO 42001",
  },
];

export default function FinancePage() {
  return (
    <>
      {/* ============================== HERO ============================== */}
      <section className="hero-dots relative border-b border-line">
        <div className="mx-auto max-w-wrap px-6 py-[clamp(48px,7vw,88px)]">
          <p className="eyebrow mb-4">For finance and fintech AI vendors</p>
          <h1 className="max-w-[20ch] font-display text-[clamp(36px,5.6vw,60px)] font-extrabold leading-[1.0] tracking-[-0.035em] text-ink">
            Your agent cleared the demo. Then it hit the bank&rsquo;s
            third-party risk review.
          </h1>
          <p className="mt-6 max-w-[58ch] font-sans text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-ink-2">
            An autonomous agent that touches account data, payments or customer
            money does not clear a financial institution on a questionnaire. The
            vendor-security team, the model-risk function and the third-party
            risk reviewer each want evidence they can check. kolm hands you a
            signed evidence report they verify offline, against your own key,
            mapped to SOC 2, the NIST AI RMF and the model-risk language they
            already use. The review compresses back to days.
          </p>

          <div className="mt-7 flex flex-wrap gap-3">
            <Button asChild>
              <Link href="/signup">Start free</Link>
            </Button>
            <Button asChild variant="ghost">
              <Link href="/contact">Talk to us</Link>
            </Button>
            <Button asChild variant="ghost">
              <Link href="/sample">See a sample report</Link>
            </Button>
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px] text-ink-3">
            {TRUST.map((t) => (
              <span key={t} className="inline-flex items-center gap-2">
                <CheckIcon className="h-3.5 w-3.5 text-[var(--accent)]" />
                {t}
              </span>
            ))}
          </div>
        </div>
      </section>

      {/* ====================== FRAMEWORK STRIP ====================== */}
      <section className="border-b border-line">
        <div className="mx-auto max-w-wrap px-6 py-12">
          <div className="flex flex-wrap items-center gap-3">
            <Badge variant="verified">Mapped to</Badge>
            {FRAMEWORKS.map((f) => (
              <span key={f} className="ctrlid">
                {f}
              </span>
            ))}
          </div>
          <p className="mt-5 max-w-[74ch] text-[13.5px] leading-relaxed text-ink-3">
            kolm maps your agent&rsquo;s controls to the frameworks your buyer
            cites. It does not issue a certification or an audit opinion, and it
            tests and reports injection resistance rather than warranting it.
          </p>
        </div>
      </section>

      {/* ============================ THE STALL =========================== */}
      <section className="border-b border-line">
        <div className="mx-auto max-w-wrap px-6 py-[96px]">
          <div className="mb-12 max-w-[66ch]">
            <p className="eyebrow mb-3">01 / The stall</p>
            <h2 className="font-display text-[clamp(28px,3.8vw,42px)] font-bold leading-[1.08] tracking-[-0.028em] text-ink">
              A bank will not take your word for it.
            </h2>
            <p className="mt-4 max-w-[52ch] text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-ink-2">
              The day your agent can act on customer money, a one-week review
              runs four to eight. The contract sits in procurement, the champion
              goes quiet, and three separate desks each want something they can
              verify instead of read.
            </p>
          </div>
          <dl className="grid gap-4 sm:grid-cols-3">
            {METRICS.map((m) => (
              <div
                key={m.l}
                className="rounded-lg border border-line bg-card p-5"
              >
                <dt className="font-display text-[clamp(26px,3vw,34px)] font-extrabold leading-none tracking-[-0.03em] text-ink">
                  {m.n}
                </dt>
                <dd className="mt-3 text-[14px] leading-relaxed text-ink-2">
                  {m.l}
                </dd>
              </div>
            ))}
          </dl>
        </div>
      </section>

      {/* ====================== WHAT THE REVIEWER ASKS ===================== */}
      <section className="border-b border-line">
        <div className="mx-auto max-w-wrap px-6 py-[96px]">
          <div className="mb-12 max-w-[66ch]">
            <p className="eyebrow mb-3">02 / What the reviewer asks</p>
            <h2 className="font-display text-[clamp(28px,3.8vw,42px)] font-bold leading-[1.08] tracking-[-0.028em] text-ink">
              Four questions a financial reviewer will not skip.
            </h2>
            <p className="mt-4 max-w-[52ch] text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-ink-2">
              Each maps to a control your buyer already enforces, and each is
              answered with a signed finding rather than a sentence you wrote.
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {ASKS.map((a) => (
              <Card key={a.title}>
                <a.icon className="h-5 w-5 text-[var(--accent)]" />
                <CardKicker className="mt-3">{a.kicker}</CardKicker>
                <CardTitle className="mt-2">{a.title}</CardTitle>
                <CardDescription className="mt-2">{a.body}</CardDescription>
                <p className="mt-4 font-mono text-[12px] text-ink-3">{a.map}</p>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* ===================== THE SIGNED REPORT (LEDGER) ================= */}
      <section className="relative border-b border-line bg-ink-deep text-on-ink">
        <div className="mx-auto max-w-wrap px-6 py-[96px]">
          <div className="mb-12 max-w-[66ch]">
            <p className="eyebrow mb-3 text-on-ink-3">03 / The report</p>
            <h2 className="font-display text-[clamp(28px,3.8vw,42px)] font-bold leading-[1.08] tracking-[-0.028em] text-on-ink">
              What you hand the model-risk and vendor-security teams.
            </h2>
            <p className="mt-4 max-w-[52ch] text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-on-ink-2">
              Not a promise. One canonical object, signed with Ed25519, mapped
              to the controls the review group already cites.
            </p>
          </div>
          <div className="grid gap-3 md:grid-cols-3">
            {HANDOFF.map((h) => (
              <Card key={h.title} ledger>
                <span className="font-sans text-[18px] font-semibold text-on-ink">
                  {h.title}
                </span>
                <p className="mt-2 text-[14px] leading-relaxed text-on-ink-2">
                  {h.body}
                </p>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* ============================ CROSSWALK =========================== */}
      <section className="border-b border-line">
        <div className="mx-auto max-w-wrap px-6 py-[96px]">
          <div className="mb-10 max-w-[66ch]">
            <p className="eyebrow mb-3">04 / The crosswalk</p>
            <h2 className="font-display text-[clamp(28px,3.8vw,42px)] font-bold leading-[1.08] tracking-[-0.028em] text-ink">
              Every finding lands in a control your buyer already enforces.
            </h2>
            <p className="mt-4 max-w-[52ch] text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-ink-2">
              Six agent-security controls map directly to SOC 2, the NIST AI RMF
              and the model-risk language your reviewers cite, so a finding never
              needs translating.
            </p>
          </div>
          <div className="overflow-x-auto rounded-lg border border-line">
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-line bg-paper-2">
                  <th className="px-4 py-3 font-sans text-[12px] font-semibold uppercase tracking-[0.08em] text-ink-3">
                    Control
                  </th>
                  <th className="px-4 py-3 font-sans text-[12px] font-semibold uppercase tracking-[0.08em] text-ink-3">
                    What it checks
                  </th>
                  <th className="px-4 py-3 font-sans text-[12px] font-semibold uppercase tracking-[0.08em] text-ink-3">
                    Maps to
                  </th>
                </tr>
              </thead>
              <tbody>
                {CROSSWALK.map((r) => (
                  <tr
                    key={r.id}
                    className="border-b border-line align-top last:border-0"
                  >
                    <td className="px-4 py-3 text-[14px] text-ink">
                      <b className="font-semibold">{r.id}</b> {r.name}
                    </td>
                    <td className="px-4 py-3 text-[14px] text-ink-2">
                      {r.checks}
                    </td>
                    <td className="px-4 py-3 font-mono text-[12.5px] text-accent-text">
                      {r.maps}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="mt-5 max-w-[80ch] text-[14px] leading-relaxed text-ink-3">
            Scope is contractual. Permission posture, redaction and audit-trail
            integrity are assessed. Injection is tested and reported, not
            warranted.
          </p>
        </div>
      </section>

      {/* ============================ CTA FINAL =========================== */}
      <section className="relative bg-ink-deep text-on-ink">
        <div className="mx-auto max-w-wrap px-6 py-[96px] text-center">
          <h2 className="mx-auto max-w-[22ch] font-display text-[clamp(28px,3.8vw,42px)] font-bold leading-[1.08] tracking-[-0.028em] text-on-ink">
            Clear the review. Close the deal.
          </h2>
          <p className="mx-auto mt-4 max-w-[52ch] text-[clamp(17.5px,1.55vw,20px)] leading-[1.55] text-on-ink-2">
            A scoped audit, a signed evidence report, and a financial security
            review that moves at the speed of math instead of the speed of a
            questionnaire.
          </p>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            <Button asChild>
              <Link href="/signup">Start free</Link>
            </Button>
            <Button
              asChild
              variant="ghost"
              className="border-[var(--line-ink-2)] text-on-ink hover:border-on-ink hover:bg-[rgba(236,239,234,0.06)]"
            >
              <Link href="/contact">Talk to us</Link>
            </Button>
          </div>
        </div>
      </section>
    </>
  );
}
