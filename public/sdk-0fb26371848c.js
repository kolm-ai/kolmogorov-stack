...
// Verified Inference (Lane 2), (3) explicit provider capture proxy calls.
...
class Recipe {
 ...
 if (this.apiKey && !hasAuth) out.Authorization = 'Bearer ' + this.apiKey;
 return out;
 }
 ...
 // Lane 3: provider capture proxy. The route authenticates with your Kolm
 // key and forwards to the upstream provider with x-upstream-api-key.
 async capture(provider, payload, headers = {}) {
 const p = String(provider || '').toLowerCase();
 const routes = {
 openai: '/v1/capture/openai',
 anthropic: '/v1/capture/anthropic',
 openrouter: '/v1/capture/openrouter',
 };
 const route = routes[p];
 if (!route) throw new Error('unsupported capture provider: ' + provider);
 const r = await fetch(this.base + route, {
 method: 'POST',
 headers: this._jsonHeaders(headers),
 ...
 return this.capture(provider, payload, headers);
 }
