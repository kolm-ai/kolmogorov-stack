...
- Homepage claim (`public/sdks.html` line 205): `$ pip install git+https://github.com/kolm-ai/kolmogorov-stack@main#subdirectory=sdk/python` plus comment "PyPI pending"
- Status: **NOT-PUBLISHED**
- Constraint: name collision on PyPI is a hard blocker — a user who runs `pip install git+https://github.com/kolm-ai/kolmogorov-stack@main#subdirectory=sdk/python` lands on the unrelated Korean LM toolkit. Homepage handles this correctly (git-from-source install line), but the implicit `kolm` rename is still unresolved.
...
- Homepage claim (`public/sdks.html` line 240): `$ curl -L -o kolm.h https://github.com/kolm-ai/kolmogorov-stack/raw/main/sdk/c/kolm.h`
...
- Package name in manifest: `kolm`
- Version: `0.2.0`
- `Cargo.toml` repository URL: `https://github.com/kolm-ai/kolm-stack` (note: differs from the actual repo `kolm-ai/kolmogorov-stack` — manifest points at a placeholder org that may not exist)
...
- Constraints:
  ...
  3. `Cargo.toml` repository URL points at `github.com/kolm-ai/kolm-stack` while the real monorepo is `github.com/kolm-ai/kolmogorov-stack`. A `cargo publish` from this manifest would ship the wrong repo link.
...
4. **Repo URL mismatch in Rust Cargo.toml.** `kolm-ai/kolm-stack` vs. real `kolm-ai/kolmogorov-stack`. Fix before any `cargo publish`.
