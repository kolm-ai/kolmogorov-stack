...
const verbs = [
  {
    name: 'registry',
    title: 'kolm registry',
    desc: 'Push, pull, list, or sign artifacts in a registry (public hub or private).',
    usage: 'kolm registry <push|pull|list|sign|info> [args]',
    flags: [
      ['push <artifact>', 'sub', 'Push an artifact to the registry.'],
      ['pull <slug>', 'sub', 'Pull an artifact by slug (also <code>kolm pull</code>).'],
      ['list', 'sub', 'List artifacts in the active registry.'],
      ['sign <artifact>', 'sub', 'Sign an artifact with the active tenant key.'],
      ['info <slug>', 'sub', 'Show metadata for one artifact: hash, provenance, k-score.'],
      ['--registry <url>', 'public hub', 'Registry URL. Honors $KOLM_REGISTRY_URL.'],
    ],
    examples: [
      'kolm registry push ./inbox-router.kolm --tag inbox/router:1.4.0',
      'kolm registry list --registry registry.acme.internal',
      'kolm registry info acme/inbox-router:1.4.0',
    ],
    notes: 'Every push embeds a content-addressed hash plus a signature from the active tenant key. The public hub is opt-in. Private registries can be self-hosted with the kolm-registry image.',
    see: [['kolm pull', '/docs/cli/pull'], ['kolm hub', '/docs/cli/hub'], ['kolm verify', '/docs/cli/verify']],
  },
  {
    name: 'logout',
    title: 'kolm logout',
    desc: 'Forget the saved API key for the active profile.',
    usage: 'kolm logout [--profile <name>] [--all]',
    flags: [
      ['--profile <name>', '<code>default</code>', 'Profile to forget.'],
      ['--all', 'off', 'Forget every saved profile.'],
    ],
    examples: [
      'kolm logout',
      'kolm logout --profile prod',
      'kolm logout --all',
    ],
    notes: 'Erases the key from <code>~/.kolm/credentials</code>. The remote token is not revoked; rotate it server-side from <code>/account/api-keys</code> if you suspect leak.',
    see: [['kolm login', '/docs/cli/login'], ['kolm whoami', '/docs/cli/whoami']],
  },
];
...
console.log(`write-extra-cli-docs: wrote=${wrote} skipped=${skipped}`);
