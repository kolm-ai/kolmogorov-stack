...
const WAVES = [
  ...
  },
  {
    ...
    summary: 'Six-wave finish drive (W481-W485) closing every remaining post-W479 audit item. W481 release-verify CI gates harden (--allow-logged-out for unauth hosts + manifest sync). W482 SDK catalog honesty contract: KOLM_SDKS rows now expose BOTH install_registry (`@kolmogorov/kolm-sdk` / `kolm` / `@kolmogorov/recipe-mcp` / `kolmogorov.kolm-vscode` / `kolm` crate / `kolm.h` header) AND install_source (`github:sneaky-hippo/kolmogorov-stack#path:sdk/<lang>`) so users get a working path even if the registry publish hasn\'t landed; `kolm sdk` / TUI / docs surfaces all print both lines + an "honesty:" footer. W484 `kolm update` refuses to run from a repo checkout (detected via sibling .git + package.json name === "kolmogorov-stack") because a clone running `npm i -g github:...` silently clobbers the user\'s global install; --force escape hatch documented in JSON envelope. W485 closes the 11-vs-344 OpenAPI gap — new scripts/build-openapi.cjs merges public/docs/api-routes.json (source of truth from src/router.js) into public/openapi.json on every build, hand-curated operations (LoginRequest schema on /v1/auth/login etc.) survive via merge-not-replace, shared components.responses.{JsonEnvelope,BadRequest,Unauthorized,RateLimited,ServerError} guaranteed, ≥300 operations now declared (was 11). Three new lock-in test suites: tests/wave482-sdk-catalog-honesty.test.js (11 tests pinning catalog ↔ sdk/*/manifest sync), tests/wave484-update-repo-checkout-guard.test.js (4 tests pinning refusal envelope + --force bypass + repo_checkout reason), tests/wave485-openapi-coverage.test.js (6 tests pinning every non-stub route has an op, curated schemas preserved, unique operationIds). sw.js wave490-openapi-sync-gate. Trap: the SDK package names had drifted across docs (`@kolm/sdk` in 3 surfaces, `kolmogorov-recipe` in sdk/python/README) — always reconcile against the actual manifest in sdk/<lang>/{package.json,Cargo.toml,pyproject.toml}.',
