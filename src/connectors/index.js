...
// (schema 'mcp-tool-call-1'/'mcp-tool-call-2'/'mcp-tool-call-3'). Checked BEFORE the generic OTLP sniff so a
// JSON-RPC log is never mistaken for loose spans.
function looksMcp(root, recs) {
  const rowLooksMcp = (r) => {
    const o = obj(r); if (!o) return false;
    if (o.schema === 'mcp-tool-call-1' || o.schema === 'mcp-tool-call-2' || o.schema === 'mcp-tool-call-3' ||
        o.receipt_version === 'mcp-tool-call-1' || o.receipt_version === 'mcp-tool-call-2' || o.receipt_version === 'mcp-tool-call-3') return true;
    if (o.jsonrpc === '2.0') {
