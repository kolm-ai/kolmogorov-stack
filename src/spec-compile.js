...
export async function compileSpec(spec, opts = {}) {
  validateSpec(spec);
  // W726 - workload_profile resolution + early validation. Accepted values
  // mirror src/kernel-selector.js KERNEL_PROFILES plus the 'auto' sentinel
  // (which signals "skip the chain slot, keep byte-stability of pre-W726
  // artifacts"). The opts arg wins over env so a programmatic caller can
  // explicitly pin 'auto' even when KOLM_WORKLOAD_PROFILE is set on the
  // shell. Null / undefined / omitted == 'auto' for the byte-stability
  // invariant; the explicit-string check below catches typos at input-
  // validation time instead of after a 30s training pass.
  if (opts.workload_profile == null && typeof process.env.KOLM_WORKLOAD_PROFILE === 'string'
      && process.env.KOLM_WORKLOAD_PROFILE.length > 0) {
    opts = { ...opts, workload_profile: process.env.KOLM_WORKLOAD_PROFILE };
  }
  if (opts.workload_profile != null && opts.workload_profile !== 'auto'
      && opts.workload_profile !== 'latency' && opts.workload_profile !== 'batching') {
    throw err(`workload_profile must be one of 'latency' | 'batching' | 'auto' (got ${JSON.stringify(opts.workload_profile)})`);
  }
  ensurePerUserSecret();
  ...
  // W708-6 - high-risk vertical disclaimer injection. When the spec declares a
  // vertical in {medical, legal, financial}, we surface a canonical runtime
  // disclaimer on the in-memory manifest the caller receives. This is the
  // exact text the runtime will inject around generated output. The field
  // is not hash-bound into artifact_hash (no provenance role); it is a
  // hygiene/audit signal. See /legal/liability for the policy rationale.
  const VERTICAL_DISCLAIMER_ADJECTIVE = { medical: 'medical', legal: 'legal', financial: 'financial' };
  const declaredVertical = (spec && typeof spec.vertical === 'string') ? spec.vertical.toLowerCase() : null;
  if (declaredVertical && Object.prototype.hasOwnProperty.call(VERTICAL_DISCLAIMER_ADJECTIVE, declaredVertical)) {
    const adj = VERTICAL_DISCLAIMER_ADJECTIVE[declaredVertical];
    built.manifest.vertical = declaredVertical;
    built.manifest.disclaimer_injection = `This output was generated by a distilled AI model. It is not a substitute for professional ${adj} advice. Verify with a qualified professional before acting.`;
  }

  // W726 - workload_profile manifest field + conditional hash slot.
  //
  // Mirrors the W460 / W721 / W722 / W809 byte-stability pattern: the slot
  // is keyed ONLY when the caller explicitly picked a kernel profile.
  // Absent / null / 'auto' all collapse to "no chain slot, no manifest
  // field" so pre-W726 artifacts rebuilt today produce byte-identical
  // bytes (and an identical artifact_hash).
  //
  // Implementation note: when a real profile is chosen, we patch the
  // manifest.json entry inside the freshly-built .kolm zip in-place and
  // re-sign signature.sig so loadArtifact() still accepts the artifact.
  // We do NOT re-run buildAndZip - that would re-stamp every
  // timestamp-derived field and surface as a hash drift on consecutive
  // builds. The receipt.json + Ed25519 + sigstore blocks reference
  // artifact_hash (not manifest_hash) so they remain valid without any
  // modification. The workload_profile_hash is informational and is NOT
  // bound into artifact_hash - that would require touching src/artifact.js
  // which is owned by the W807-W810 wave and not this one. A future wave
  // can promote the slot into artifact_hash via kv_profile-style threading
  // through buildAndZip.
  if (opts.workload_profile != null && opts.workload_profile !== 'auto') {
    const profile = opts.workload_profile; // 'latency' | 'batching' (validated above)
    const { default: AdmZip } = await import('adm-zip');
    // Pass 1 - read the freshly-built zip and extract every entry into a
    // plain {name, data} list. We CANNOT use AdmZip.updateFile + writeZip
    // because src/artifact.js builds the zip via the archiver streaming
    // pipeline, which emits local-file-header data descriptors (general-
    // purpose flag bit 3 set, CRC and sizes deferred to a 4-byte trailer).
    // adm-zip's writeZip strips that flag from the local header but does
    // NOT relocate the trailer, so every entry (not just the patched ones)
    // becomes unreadable on the next open. The robust path is to extract
    // ALL entries first, then rebuild the zip from scratch with adm-zip's
    // addFile API, which writes well-formed local headers without
    // descriptors. See node_modules/adm-zip/zipEntry.js:42 DESCRIPTOR_NOT_EXIST.
    const sourceZip = new AdmZip(final);
    const allEntries = sourceZip.getEntries().map(e => ({
      name: e.entryName,
      data: e.getData(), // must call BEFORE any updateFile to avoid the bug
    }));
    const manFile = allEntries.find(e => e.name === 'manifest.json');
    if (!manFile) {
      throw err(`workload_profile patch failed: manifest.json not found in built artifact ${final}`);
    }
    const sigFile = allEntries.find(e => e.name === 'signature.sig');
    if (!sigFile) {
      throw err(`workload_profile patch failed: signature.sig not found in built artifact ${final}`);
    }
    const manText = manFile.data.toString('utf8');
    const onDiskManifest = JSON.parse(manText);
    onDiskManifest.workload_profile = profile;
    onDiskManifest.workload_profile_hash = crypto.createHash('sha256')
      .update(canonicalJsonLocal({ profile }))
      .digest('hex');
    // Re-stamp the in-memory copy too so callers that read `result.manifest`
    // see the same fields the on-disk zip carries.
    built.manifest.workload_profile = profile;
    built.manifest.workload_profile_hash = onDiskManifest.workload_profile_hash;
    // Pretty-print with 2-space indent to match the existing manifest.json
    // formatting convention in src/artifact.js (which JSON.stringify(...,
    // null, 2)s every emitted manifest). Same indent keeps grep-friendly
    // diffs and preserves expectation of human-readable artifact inspection.
    const updatedManJson = JSON.stringify(onDiskManifest, null, 2);
    const newManifestHash = crypto.createHash('sha256')
      .update(Buffer.from(updatedManJson))
      .digest('hex');
    // Re-sign signature.sig so verifyManifestSignature() accepts the
    // updated manifest. The HMAC payload shape MUST match the original
    // sign-time payload in src/artifact.js (the canonicalJson of
    // {spec, manifest_hash, job_id, artifact_hash, eval_set_hash,
    // eval_score, judge_id}). The only field that changes is manifest_hash;
    // artifact_hash + eval_set_hash + eval_score + judge_id are preserved
    // from the original signature so receipt.json + credential.json +
    // Ed25519/sigstore blocks (which all reference artifact_hash and never
    // manifest_hash) stay valid without modification.
    const sigText = sigFile.data.toString('utf8');
    const sigObj = JSON.parse(sigText);
    sigObj.manifest_hash = newManifestHash;
    const ARTIFACT_SPEC_LOCAL = 'kolm-1';
    const sigPayload = {
      spec: ARTIFACT_SPEC_LOCAL,
      manifest_hash: newManifestHash,
      job_id: sigObj.job_id,
      artifact_hash: sigObj.artifact_hash,
      eval_set_hash: sigObj.eval_set_hash,
      eval_score: sigObj.eval_score,
      judge_id: sigObj.judge_id,
    };
    const signingSecret = process.env.RECIPE_RECEIPT_SECRET
      || process.env.KOLM_ARTIFACT_SECRET;
    if (!signingSecret) {
      throw err(`workload_profile patch failed: no signing secret in env (RECIPE_RECEIPT_SECRET or KOLM_ARTIFACT_SECRET)`);
    }
    sigObj.hmac = crypto.createHmac('sha256', signingSecret)
      .update(canonicalJsonLocal(sigPayload))
      .digest('hex');
    const updatedSigJson = JSON.stringify(sigObj, null, 2);
    // Pass 2 - assemble a fresh zip with the patched manifest + signature
    // and every other entry copied verbatim. addFile writes a well-formed
    // local header (no data descriptor) so the resulting .kolm is openable
    // by adm-zip, yauzl, the OS zip viewer, etc.
    const rebuilt = new AdmZip();
    for (const e of allEntries) {
      if (e.name === 'manifest.json') {
        rebuilt.addFile(e.name, Buffer.from(updatedManJson, 'utf8'));
      } else if (e.name === 'signature.sig') {
        rebuilt.addFile(e.name, Buffer.from(updatedSigJson, 'utf8'));
      } else {
        rebuilt.addFile(e.name, e.data);
      }
    }
    rebuilt.writeZip(final);
  }
