...
const TEMPLATES = Object.freeze({
  ...
  'eu-ai-act': {
    id: 'eu-ai-act',
    name: 'EU AI Act Art.9 / Art.10 / Art.11 / Art.12 / Art.14 / Art.15 checklist',
    description: 'Risk management (Art.9), data governance (Art.10), technical documentation (Art.11), record-keeping and automatic logging (Art.12), human oversight (Art.14), and accuracy, robustness and cybersecurity (Art.15) for a high-risk AI system. A mapping aid against the signed report, not a certification.',
    questions: [
      { id: 'art9-risk-management', text: 'Art.9: Is a risk-management process evidenced, including adversarial and prompt-injection testing of foreseeable misuse?', asr: ['ASR-4'] },
      ...
      { id: 'art11-technical-documentation', text: 'Art.11: Is technical documentation supported by enumerated model and dependency provenance plus verifiable evidence?', asr: ['ASR-5', 'ASR-6'] },
      ...
      { id: 'art15-accuracy-robustness-security', text: 'Art.15: Is the system resilient against adversarial manipulation and prompt injection (accuracy, robustness, cybersecurity)?', asr: ['ASR-4'] },
    ],
  },
  'ai-caiq': {
    id: 'ai-caiq',
    name: 'CSA AICM / AI-CAIQ (representative subset)',
    description: 'An AI-CAIQ-style subset spanning AI governance, identity and access, logging, data privacy and egress, adversarial testing, model and supply-chain provenance, evidence verifiability, retrieval and memory integrity, and agent delegation. A mapping aid against the signed report, not a certification.',
    questions: [
      { id: 'aicm-gov-01', text: 'Is there an AI governance program assigning accountability for agent behaviour and least-privilege risk treatment?', asr: ['ASR-1'] },
      { id: 'aicm-iam-01', text: 'Are agent and service entitlements granted on a least-privilege basis?', asr: ['ASR-1'] },
      { id: 'aicm-iam-02', text: 'Is each agent identity assigned a unique credential, with no shared keys across isolation boundaries?', asr: ['ASR-1'] },
      { id: 'aicm-log-01', text: 'Are AI-system events logged with enough detail to attribute who did what and when?', asr: ['ASR-2'] },
      { id: 'aicm-log-02', text: 'Are audit logs tamper-evident and retained per a documented policy?', asr: ['ASR-2'] },
      { id: 'aicm-dsp-01', text: 'Is sensitive and personal data identified and protected against unauthorized egress to third parties?', asr: ['ASR-3'] },
      { id: 'aicm-ais-01', text: 'Is the AI system tested against adversarial input, including direct and indirect prompt injection?', asr: ['ASR-4'] },
      { id: 'aicm-sta-01', text: 'Are model versions, MCP servers, and dependencies inventoried, pinned, and provenance-tracked?', asr: ['ASR-5'] },
      { id: 'aicm-aud-01', text: 'Is AI-security evidence cryptographically signed and independently verifiable offline?', asr: ['ASR-6'] },
      { id: 'aicm-dsp-02', text: 'Are retrieval and memory sources enumerated and trusted, with memory writes attributable?', asr: ['ASR-7'] },
      { id: 'aicm-gov-02', text: "Is every multi-agent delegation attributable and attenuated to a subset of the delegating agent's authority?", asr: ['ASR-8'] },
    ],
  },
  'sig-core': {
    id: 'sig-core',
    name: 'SIG Core (representative subset)',
    description: 'A broader SIG-style set than SIG Lite, across access control, logging and monitoring, data security, threat and vulnerability management including prompt injection, supply chain, and AI-specific governance. A mapping aid against the signed report, not a certification.',
    questions: [
      { id: 'sigc-ac-01', text: 'Is logical access to systems, tools, and data restricted on a least-privilege basis?', asr: ['ASR-1'] },
      { id: 'sigc-ac-02', text: 'Is each service or agent identity assigned a unique credential, with no shared keys across boundaries?', asr: ['ASR-1'] },
      { id: 'sigc-ac-03', text: 'Are high-privilege or destructive actions gated behind step-up controls or human approval?', asr: ['ASR-1'] },
      { id: 'sigc-log-01', text: 'Are security-relevant events logged with enough detail to establish who did what and when?', asr: ['ASR-2'] },
      { id: 'sigc-log-02', text: 'Are audit logs protected against unauthorized modification (tamper-evident)?', asr: ['ASR-2'] },
      { id: 'sigc-log-03', text: 'Are audit logs retained in line with a documented retention policy?', asr: ['ASR-2'] },
      { id: 'sigc-ds-01', text: 'Is sensitive data identified and protected against unauthorized egress to third parties?', asr: ['ASR-3'] },
      { id: 'sigc-ds-02', text: 'Are retrieval and memory sources enumerated and trusted, with memory writes attributable?', asr: ['ASR-7'] },
      { id: 'sigc-tv-01', text: 'Do you perform adversarial testing of the application, including direct and indirect prompt injection?', asr: ['ASR-4'] },
      { id: 'sigc-sc-01', text: 'Are third-party, model, and MCP dependencies inventoried, pinned, and version-controlled?', asr: ['ASR-5'] },
      { id: 'sigc-ai-01', text: 'Is AI-security evidence signed and independently verifiable offline?', asr: ['ASR-6'] },
      { id: 'sigc-ai-02', text: "Is every multi-agent delegation attributable and attenuated to a subset of the delegating agent's authority?", asr: ['ASR-8'] },
    ],
  },
  vsaq: {
    id: 'vsaq',
    name: 'VSAQ-style vendor security questionnaire (representative subset)',
    description: 'A VSAQ-style vendor questionnaire across security controls, privacy and data handling, data egress, evidence, and breach notification. A mapping aid against the signed report, not a certification.',
    questions: [
      { id: 'vsaq-sec-01', text: 'Is access to systems and data restricted on a least-privilege basis with unique per-identity credentials?', asr: ['ASR-1'] },
      { id: 'vsaq-sec-02', text: 'Are security-relevant events logged and retained to support investigation?', asr: ['ASR-2'] },
      { id: 'vsaq-priv-01', text: 'Is sensitive and personal data handled so that it is redacted or controlled before egress to third parties?', asr: ['ASR-3'] },
      { id: 'vsaq-priv-02', text: 'Are retrieval and memory data sources enumerated and trusted?', asr: ['ASR-7'] },
      { id: 'vsaq-sec-03', text: 'Do you test the application against adversarial input, including prompt injection?', asr: ['ASR-4'] },
      { id: 'vsaq-sc-01', text: 'Are model and third-party dependencies inventoried and pinned?', asr: ['ASR-5'] },
      { id: 'vsaq-breach-01', text: 'Are logs tamper-evident and verifiable enough to support breach notification and forensic reconstruction?', asr: ['ASR-2', 'ASR-6'] },
    ],
  },
  'eu-ai-act-fria': {
    id: 'eu-ai-act-fria',
    name: 'EU AI Act FRIA (fundamental-rights impact, representative)',
    description: 'A representative fundamental-rights impact assessment aid covering intended purpose, affected persons, risk to rights, human oversight, record-keeping, data governance, and accuracy, robustness and security for a high-risk AI system. A mapping aid, not a completed FRIA.',
    questions: [
      { id: 'fria-purpose', text: 'Is the intended purpose of the system documented, and are destructive or high-privilege actions gated for human oversight of that purpose?', asr: ['ASR-1'] },
      { id: 'fria-affected-persons', text: 'Are the categories of persons affected by the system identified, with each agent action attributable to a declared identity?', asr: ['ASR-1'] },
      { id: 'fria-risk-to-rights', text: 'Is the risk that sensitive personal data leaves the boundary and harms affected persons controlled?', asr: ['ASR-3'] },
      { id: 'fria-human-oversight', text: 'Is each multi-agent delegation attributable and attenuated so a human can oversee decisions affecting rights?', asr: ['ASR-8'] },
      { id: 'fria-record-keeping', text: 'Are events automatically recorded so the impact on rights can be reviewed after the fact?', asr: ['ASR-2'] },
      { id: 'fria-data-governance', text: 'Are input data governed and retrieval and memory sources enumerated and trusted?', asr: ['ASR-3', 'ASR-7'] },
      { id: 'fria-accuracy-robustness-security', text: 'Is the system resilient against adversarial manipulation and prompt injection that could harm affected persons?', asr: ['ASR-4'] },
    ],
  },
});
